...
"""
Shared diff, display, and apply logic for syncing incident reports to Jira.
"""
...
from iim.libreport import ActionItem, IncidentReport, jira_key
...
def print_diff_table(
    ...
    click.echo()
    click.echo(f"Jira: {jira_incident.jira_url}")
...
def apply_changes(
    jira_client: JiraAPI,
    issue_key: str,
    status_diff: list[Diff],
    metadata_diff: list[Diff],
    actions_diff: list[Diff],
) -> None:
    """Apply diff changes to Jira. Silent — no printing."""
